...
class Solution42 {
    ...
    public static int trap2(int[] height) {
        ...
        for (int i = 1; i < height.length - 1; i++) {
            ...
            for (int j = i - 1; j >= 0; j--) if (height[j] > maxLeft) maxLeft = height[j];
            for (int j = i + 1; j < height.length; j++) if (height[j] > maxRight) maxRight = height[j];
     ...
     * 优化上面的方法
     * 原理；
     * 双指针，左侧指针设为1，右侧指针设为数组长度-2，然后遍历整个数组，每次取元素的左邻居
     * 和右指针的右邻居进行比较：
